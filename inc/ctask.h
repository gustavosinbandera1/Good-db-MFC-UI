...
class GOODS_DLL_EXPORT task_queue { 
    friend class task;
...
class GOODS_DLL_EXPORT event_internals : public semaphore_internals { 
  protected: 
    friend class task;
    friend class task_internals;
...
class GOODS_DLL_EXPORT mutex_internals : public semaphore_internals {
  protected: 
...
class GOODS_DLL_EXPORT semaphorex_internals : public semaphore_internals { 
  protected: 
    mutex_internals& cs;
    ...
    boolean wait_with_timeout(time_t sec) { 
        if (count > 0) { 
            ...
            assert(cs.nest == 0);
            cs.release();
            boolean result = semaphore_internals::wait_with_timeout(sec);
            cs.enter();
            return result;
        }       
    }
...
class GOODS_DLL_EXPORT eventex_internals : public event_internals { 
  protected: 
    mutex_internals& cs;
    ...
    boolean wait_with_timeout(time_t sec) { 
        ...
        assert(cs.nest == 0);
        cs.release();
        boolean result = event_internals::wait_with_timeout(sec);
        cs.enter();
        return result;
    }   
...
class GOODS_DLL_EXPORT ctimer : public l2elem { 
    friend class async_event_manager;
  protected: 
    time_t            start;    // time of timer starting
    time_t            timeout;  
    void*             arg;
    ctimer_callback_t f;
  ...
  public:
    ...
    void start_timer(ctimer_callback_t f, void* arg, time_t timeout); 
...
extern "C" int  debug_list_tasks();
extern "C" int  debug_select_task(int i);
extern "C" int  debug_select_task_internals(task_internals *tp);
...
class GOODS_DLL_EXPORT task_internals { 
    ...
    friend int debug_list_tasks();
    friend int debug_select_task_internals(task_internals *tp);
  ...
  protected: 
    ...
    nat8            profile_start; // micro-second when thread goes active
    nat8            profile_usecs; // count of micro-seconds during which
                                   // thread is active
    ...
    //
    boolean wait(semaphore_internals* sem, time_t timeout);
    ...
    static void* get_task_specific() {
        return curr->task_specific_data;    
    }

    static void  set_task_specific(void* data) { 
        curr->task_specific_data = data;
    }
