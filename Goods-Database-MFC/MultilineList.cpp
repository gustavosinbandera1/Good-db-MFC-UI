...
// Win32 class name used for the window; if you're using the VS dialog editor
// you need to put this string in the Class property of the custom control
...
CMultilineList::CMultilineList()
    : m_nCols(0), m_nRows(0), m_viewYPos(0), m_viewXPos(0), m_curSelRow(-1) {
  RegisterWindowClass();
}
...
void CMultilineList::SetSize(int nCols, int nRows) {
  ...
  if ((nRows < m_nRows) && (nCols >= m_nCols)) {
    ...
    for (k = m_rowHeights.begin(); k != m_rowHeights.end();) {
      ...
      // remove (invalidate) calculated row heights corresponding to rows which
      // are outside the new dimensions
      if (row >= nRows) {
...
int CMultilineList::GetRowFromPoint(CPoint pt) {
  ...
  if (cr.PtInRect(pt)) {
    ...
    // walk through the rows until we find one at the specified vertical
    // location
    int row = 0;
...
void CMultilineList::CreateChildControls() {
  m_headerCtrl.Create(CCS_TOP | WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), this,
                      CHILD_ID_HEADERCTRL);
  m_headerCtrl.SetFont(&m_headerFont);

  m_scrollBar.Create(SBS_RIGHTALIGN | SBS_VERT | WS_CHILD | WS_VISIBLE,
                     CRect(0, 0, 0, 0), this, CHILD_ID_SCROLLBAR);

  m_scrollBarHorz.Create(SBS_BOTTOMALIGN | SBS_HORZ | WS_CHILD | WS_VISIBLE,
...
void CMultilineList::UpdateChildControls() {
  ...
  for (int col = 0; col < m_nCols; col++) {
    ...
    if (m_headerCtrl.GetItem(col, &hdi)) {
      ...
      if ((curHeading != column.m_heading) || (hdi.cxy != column.m_width)) {
  ...
  for (iRowHeights = m_rowHeights.begin(); iRowHeights != m_rowHeights.end();
       ++iRowHeights) {
  ...
  if (totalHeight <= contentRect.Height()) {
    ...
    if ((nMin != si.nMin) || (nMax != si.nMax) || (nPage != si.nPage) ||
        (nPos != si.nPos)) {
      si.nMin = nMin;
      si.nMax = nMax;
      si.nPage = nPage;
  ...
  for (int row = 0; row < m_nRows; row++) {
    ...
    if (iRow == m_rowHeights.end()) {
      ...
      for (int col = 0; col < m_nCols; col++) {
        ...
        Cell cell;
        map<pair<int, int>, Cell>::iterator iCell =
            m_cells.find(make_pair(col, row));
        ...
        // compute cell height
        m_offscreenDC.DrawTextEx(
            cell.m_text, textRect,
            DT_CALCRECT | DT_LEFT | DT_NOPREFIX | DT_TOP | DT_WORDBREAK, NULL);
...
void CMultilineList::OnVScroll(UINT nSBCode, UINT nPos,
  ...
  if ((nSBCode == SB_THUMBPOSITION) || (nSBCode == SB_THUMBTRACK)) {
    ...
    // determine remaining height in list (including and after the top visible
    // row)
...
void CMultilineList::OnHScroll(UINT nSBCode, UINT nPos,
                               CScrollBar *pScrollBar) {
...
void CMultilineList::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  GetParent()->PostMessage(WM_COMMAND, MAKEWPARAM(thisCtrlID, LBN_SELCHANGE),
                           (LPARAM)GetSafeHwnd());
}
...
void CMultilineList::OnPaint() {
  ...
  dc.BitBlt(renderRect.left, renderRect.top, renderRect.Width(),
            renderRect.Height(), &m_offscreenDC, renderRect.left,
            renderRect.top, SRCCOPY);
...
void CMultilineList::RenderContent(CDC &dc, CRect &r) {
  ...
  for (; (row < m_nRows) && (y1 <= r.bottom); row++) {
    ...
    for (int col = 0, x1 = r.left - m_viewXPos; (col < m_nCols); col++) {
      ...
      // if this row is selected, fill in the background with the selection
      // color and set the text color
      if (row == m_curSelRow) {
        ...
        dc.FillRect(bgRect,
                    CBrush::FromHandle(GetSysColorBrush(COLOR_HIGHLIGHT)));
      ...
      dc.DrawTextEx(cell.m_text, textRect,
                    DT_LEFT | DT_NOPREFIX | DT_TOP | DT_WORDBREAK, NULL);
    ...
    // if not the first row, also render the top horizontal line above the
    // current row
    ...
    for (int col = 0, x1 = r.left - m_viewXPos;
         (col < m_nCols) && (x1 <= r.right); col++) {
      Column column;
      std::map<int, Column>::iterator i = m_columns.find(col);
      if (i != m_columns.end()) {
        column = i->second;
